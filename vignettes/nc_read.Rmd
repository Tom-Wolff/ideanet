...
Interviewer
The first step in exporting data from Interviewer is to navigate to the Manage and Export Session data in the Interviewer app.
...
Fresco
For information on how to export data in Fresco, see using Fresco.

Import and clean the data
Next we want to load the data into R. We use the egor to do this, using examples derived from an egor vignette.

- Data structure produced by Network Canvas
* CSV files
* Ego level (egos)
* Alter level (alters)
* Alter-alter ties (edgelist)

# Using `nc_read`

ONLY ONE NC PROTOCOL AT A TIME

`nc_read` takes two arguments:

* `path`: A character value indicating the directory in which Network Canvas CSVs are located.
* `protocol`: A character value indicating the pathname of the Network Canvas protocol file corresponding to the data being read. Reading in the protocol is optional but recommended for accurate encoding of categorical variables.
...
COORDINATES STORED IN ALTER LIST-- DOES THIS WORK FOR PLOTTING IN IGRAPH?
