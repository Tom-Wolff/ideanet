...
Next, we run the `CHAMP` algorithm on the resulting partitions, and plot results to visualize the domains of optimality in the resolution parameter space for each somewhere-optimal partition. Note that `CHAMP` updates the `partitions` list with a `CHAMPsummary` and `CHAMPfigure`.
```{r}
