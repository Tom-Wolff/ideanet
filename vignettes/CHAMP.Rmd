...
flor_partitions <- CHAMP(nw_flor$florentine, flor_partitions, plottitle = "Florentine (combined)")
```
...
flor_partitions <- get_CHAMP_map(nw_flor$florentine, flor_partitions, plotlabel = "Florentine (combined)")
```
...
```{r partition8}
kc_partitions$partitions[[8]]
igraph::membership(kc_partitions$partitions[[8]])
```

```{r partition8viz, eval = FALSE}
igraph::plot.igraph(karate,mark.groups = kc_partitions$partitions[[8]])
```

### Step 3: Compute the SBM-equivalence iterative map on the CHAMP set

```{r, warning=FALSE}
kc_partitions <- get_CHAMP_map(karate, kc_partitions, plotlabel = "Zachary's Karate Club (unweighted)")
```
...
```
```{r karate_partitions2, warning=FALSE}
kcw_partitions <- get_partitions(karate, n_runs = 500, seed = 3478)
kcw_partitions <- CHAMP(karate,kcw_partitions,plottitle="Zachary's Karate Club (weighted)")
```
```{r karate_map2, warning=TRUE}
kcw_partitions <- get_CHAMP_map(karate,kcw_partitions,plotlabel="Zachary's Karate Club (weighted)")
```

```{r karate_CHAMPsummary3, eval = FALSE}
print(kcw_partitions$CHAMPsummary)
```

```{r karate_CHAMPsummary3_kable, echo = FALSE}
knitr::kable(kcw_partitions$CHAMPsummary)
```
...
```{r karate2_partitionviz, eval = FALSE}
karatelayout <- igraph::layout_with_fr(karate)
igraph::plot.igraph(karate, layout = karatelayout,
                    mark.groups = kcw_partitions$partitions[[4]])
igraph::plot.igraph(karate, layout = karatelayout,
                    mark.groups = kcw_partitions$partitions[[6]])
```

```{r karate2_partition_table}
table(igraph::membership(kcw_partitions$partitions[[4]]),
      igraph::membership(kcw_partitions$partitions[[6]]))
```
...
football_partitions <- CHAMP(football,football_partitions,plottitle="Div-IA College Football, Fall 2000")
football_partitions <- get_CHAMP_map(football,football_partitions,plotlabel="Div-IA College Football, Fall 2000")
```

```{r football_CHAMPsummary, eval = FALSE}
print(football_partitions$CHAMPsummary)
```
```{r football_CHAMPsummary_kable, echo = FALSE}
knitr::kable(football_partitions$CHAMPsummary)
```
...
```{r, warning=FALSE}
football_partitions <- get_partitions(football, n_runs = 500, gamma_range = c(0,7), seed = 115)
football_partitions <- CHAMP(football,football_partitions,plottitle="Div-IA College Football, Fall 2000")
football_partitions <- get_CHAMP_map(football,football_partitions,plotlabel="Div-IA College Football, Fall 2000")
print(football_partitions$CHAMPsummary)
```
...
As with the karate club, we highlight that the result here is different from that found in Newman (2016), where the number of communities is pre-specified to be 11 for this example. In contrast, by allowing for different possible numbers of communities, the fixed point identified here has 12 communities. At the same time, we note that extending the `gamma_range` beyond its default here identified a 14-community partition (`partition_num` 41) that is the optimum over a wide range of gamma. While this partition is not a fixed point — indeed, it points to the 12-community partition — it might be interesting in practice to further consider such a partition, since it has one of the largest domains of optimality in gamma. (We will briefly return to both the 11-community and the 14-community partitions below.)
...
igraph::plot.igraph(football,mark.groups = football_partitions$partitions[[36]],
                    vertex.label=NA, vertex.size=5)
```

```{r football_table}
table(igraph::V(football)$value,
      igraph::membership(football_partitions$partitions[[36]]))
```
...
```{r community7}
igraph::V(football)$label[football_partitions$partitions[[36]]$membership == 7]
```
...
```{r community11}
igraph::V(football)$label[football_partitions$partitions[[36]]$membership == 11]
```
aligns perfectly with the 9 teams in Conference USA, while Community 12
```{r community12}
igraph::V(football)$label[football_partitions$partitions[[36]]$membership == 12]
```
...
```{r table36_41}
table(igraph::membership(football_partitions$partitions[[36]]),
      igraph::membership(football_partitions$partitions[[41]]))
```
...
Similarly, we again note that Newman (2016) specified 11 communities. Our results above found an 11-community solution, partition_num 33, optimal for $\gamma$ between 1.20 and 1.45, that maps to a $\gamma$ estimate of 2.27 (where the 12-community partition is optimal). Since this estimated $\gamma$ matches the value in Table I of Newman (2016), we believe it is likely the same partition found there and we compare it with the 12-community fixed point:
```{r table36_33}
table(igraph::membership(football_partitions$partitions[[36]]),
      igraph::membership(football_partitions$partitions[[33]]))
```

You can see from the above table that the two partitions are almost identical, we both of them providing excellent solutions for identifying the conference structure. The only differences all involve the 5 teams in Community 10 of the 12-community partition,
```{r}
igraph::V(football)$label[football_partitions$partitions[[36]]$membership == 10]
```
