...
```{r flor_partitions}
flor_partitions <- get_partitions(nw_flor$florentine, n_runs = 2500, seed = 4781)
```
...
```{r partition_query}
flor_partitions$partitions[[6]]
flor_partitions$partitions[[9]]
```
...
```{r partition6}
flor_partitions$partitions[[6]]
igraph::membership(flor_partitions$partitions[[6]])
```

```{r parition6_viz, eval = FALSE}
igraph::plot.igraph(nw_flor$florentine,mark.groups = flor_partitions$partitions[[6]])
```
...
```{r karate_partitions}
kc_partitions <- get_partitions(karate, n_runs = 2500, seed = 3478)
```
...
kcw_partitions <- get_partitions(karate, n_runs = 2500, seed = 3478)
...
```{r, warning=FALSE}
football_partitions <- get_partitions(football, n_runs = 1000, seed = 115)
...
```{r, warning=FALSE}
football_partitions <- get_partitions(football, n_runs = 2000, gamma_range = c(0,7), seed = 115)
football_partitions <- CHAMP(football,football_partitions,plottitle="Div-IA College Football, Fall 2000")
football_partitions <- get_CHAMP_map(football,football_partitions,plotlabel="Div-IA College Football, Fall 2000")
print(football_partitions$CHAMPsummary)
```
