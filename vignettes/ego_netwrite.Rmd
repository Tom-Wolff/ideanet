...
```
Our ego list contains information for the 10 egos in our dataset. The ego list also has information regarding the age, sex, race/ethnicity, educational attainment, and political leanings of each ego.
...
```{r ego_search}
which(lapply(igraph_objects, function(x){x$ego} == 4) == TRUE)
```
...
igraph_objects[[4]]$ego_info
```

```{r ego_info_kable, echo = FALSE}
knitr::kable(igraph_objects[[4]]$ego_info)
```
...
ego4 <- igraph_objects[[4]]$igraph_ego
