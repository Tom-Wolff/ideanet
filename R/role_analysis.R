...
#'
#' @description The `role_analysis` function takes networks processed by the `netwrite` function and performs positional analysis on them. Positional analysis methods allows users to infer distinct "roles" in networks from patterns in network activity. `role_analysis` currently supports the identification of roles using two methods: hierarchical clustering (cite) and convergence of correlations (CONCOR, Breiger 1975).
#'
#' @param graph An igraph object or a list of igraph objects produced as output from `netwrite`.
#' @param nodes A data frame containing individual-level network measures for each node in the network. Ideally, the `node_measures` data frame produced by `netwrite` should be assigned to this argument.
#' @param directed A logical value indicating whether network edges should be treated as direected.
#' @param method A character value indicating the method used for positional analysis. Valid arguments are currently `"cluster"` for hierarchical clustering and `"concor"` for CONCOR.
...
#' @param min_partition_size A numeric value indicating the minimum number of nodes required for inclusion in a cluster. If an inferred cluster or partition contains fewer nodes than the number assigned to `min_partition_size`, nodes in this cluster/partition will be labeled as members of a parent cluster/partition.
