...
get_partitions <- function( network,
  ...
  # appropriately for the following steps. The function can also removes
...
unique_partitions2 <- function(partitions) {

  unique_list <- list()
  # Create matrix of match scores
  match_mat <- matrix(rep(NA, length(partitions)^2), ncol = length(partitions))
  ...
  for (i in 1:length(partitions)) {
    for (j in 1:length(partitions)) {
      ...
      # match_mat[i, j] <- igraph::compare(partitions[[i]], partitions[[j]], method = "adjusted.rand")
        match_mat[i, j] <- identical(partitions[[i]]$membership, partitions[[j]]$membership)
      }
    }
  }
