...
get_CHAMP_map <- function( network, 
  ...
  if (igraph::is_weighted(network)) {
    warning("The theory underlying get_CHAMP_map() is for unweighted networks. The formulae have been naturally generalized to weighted networks, but have not been well studied.")
  }
  
  partition_summary <- partitions$CHAMPsummary #as generated by CHAMP.R
  
  for (x in 1:nrow(partition_summary)) {
    if (partition_summary[x, "num_communities"] > 1) {
