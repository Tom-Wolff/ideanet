...
get_CHAMP_map <- function( network,
  ...
  for (x in 1:nrow(partition_summary)) {
    if (partition_summary[x, "num_communities"] > numcomponents) {
      ...
      res_param <- if (partition$nb_clusters > 1) {
        
        # get memberships for this partition
        mem <- partition$membership
        
        # calculate sum of community degrees
        ...
        # calculate external edge weight
        m.out <- m - m.in
        
        theta.in <- 2 * m.in / (k2.c / (2 * m))
        theta.out <- 2 * m.out / (2 * m - k2.c / (2 * m))
        
        (theta.in - theta.out) / (log(theta.in) - log(theta.out))
      ...
      if (!is.null(res_param) && !is.na(res_param)) {
        ...
        if (length(idx) > 0) {
          partition_summary[x, "next_partition_num"] <- 
  ...
  for (x in 1:nrow(partition_summary)) {
    if (!is.na(partition_summary[x, "next_gamma"]) &&
      ...
      #     network,
      #     main = str_c("Partition", pnum, "with", 
      #                  partition_summary[x, "num_communities"],
      #                  "clusters", sep = " "))
      print(paste("Partition #", pnum, "(with",
                  partition_summary[x,"num_communities"],
                  "communities) is a fixed point of the iterative map"))
      fixed_points <- append(fixed_points, pnum)
    }
  }
  partitions$fixedpoints <- fixed_points
  ...
  if (isTRUE(plots)) {
    ...
    ggfig <- ggplot2::ggplot(data = plot_data.1) +
      ggplot2::geom_line(ggplot2::aes(x = .data$x, y = .data$y, group = .data$group, color=palette[.data$color]), linewidth=2) +
      ggplot2::geom_point(ggplot2::aes(x = .data$x, y = .data$y, color=palette[.data$color]), shape=4, size=2, stroke=2) +
