...
#' @param partitions A list of unique partitions (in the format generated by \code{\link{get_partitions}}), a list object produced by \code{\link{comm_detect}}, or a list containing both of these list objects.
#' @param plottitle Optional title for generated plot of (generalized) modularity versus resolution parameter.
#'
#' @returns \code{CHAMP} returns the input list of partitions with a \code{$CHAMPsummary} about which partitions are somewhere optimal (in the sense of modularity Q with a resolution parameter gamma) and their domains of optimality, along with the generated \code{$CHAMPfigure} plot of the upper envelope of Q(gamma). The returned list object also contains the original list(s) entered into the \code{partitions} argument.
#'
#'
#' @export
#'
...
# In the original CHAMP paper and python implementation, we use QHull to do the heavy lifting. But since we're only dealing with a 1D parameter space here (for single-layer networks; multilayer networks will be dealt with elsewhere), we can instead brute-force the identification of the upper envelope of the Q(gamma) lines. (See the paper for figures of this.) Importantly, our simple algorithm here searching for the next line crossing could fail in the (rare?) circumstance where three different partitions intersect on the upper envelope of Q(gamma) at the same point.
  ...
  # Most thorough way of identifying which input object we're dealing with here
  if (min(c("count", "partitions", "gamma_min", "gamma_max") %in% names(partitions)) == 1) {
    partitions_type <- "get_partitions"
  } else if (min(c("memberships", "summaries", "score_comparison") %in% names(partitions)) == 1) {
    partitions_type <- "comm_detect"
  } else if (length(partitions) == 2) {
    partitions_type <- "joint"
  } else {
    stop("Input for `partitions` argument is invalid. Please use either an object created by `get_partitions`, `comm_detect`, or a list object containing one of each of these items.")
  }

  if (partitions_type == "joint") {
    # Identify which of the two items is the `comm_detect` output
    if (min(c("memberships", "summaries", "score_comparison") %in% names(partitions[[1]])) == 1) {
      comm_detect_obj <- partitions[[1]]
      get_part_obj <- partitions[[2]]
    } else {
      comm_detect_obj <- partitions[[2]]
      get_part_obj <- partitions[[1]]
    }

    partitions$gamma_min <- get_part_obj$gamma_min
    partitions$gamma_max <- get_part_obj$gamma_max

    ### Remove any methods that were skipped in `comm_detect`
    membership_df <- comm_detect_obj$memberships[, !(colSums(is.na(comm_detect_obj$memberships)) == nrow(comm_detect_obj$memberships))]

    for (k in 1:length(get_part_obj$partitions)) {
      membership_df[,ncol(membership_df)+1] <- igraph::membership(get_part_obj$partitions[[k]])
      colnames(membership_df)[ncol(membership_df)] <- paste("gp", k, sep = "_")
    }
  ...
  # `comm_detect`
  if (partitions_type == "get_partitions") {
  ...
  } else if (partitions_type == "comm_detect") {

    partitions$gamma_min <- 0
    partitions$gamma_max <- 3
    ...
    mod_matrix <- data.frame(row.names = colnames(membership_df)[-1],
                             base = 2:ncol(membership_df) - 1,
                             decrement = 2:ncol(membership_df) - 1)

    for (k in 2:ncol(membership_df)) {
      #VERY SLOW: ret <- partition_level_champ(network, partition)
      #mod_matrix[k, "base"] <- ret$a_partition
      #mod_matrix[k, "decrement"] <- ret$p_partition

      # Jim says to make all the zero cases their own class, just add 1 to all values
      # in mem_vec and give a warning
      mem_vec <- as.vector(membership_df[,k])

      if (0 %in% mem_vec) {
        mem_vec <- mem_vec + 1
      }
      # mem_vec[which(mem_vec == 0)] <- max(mem_vec) + 1:sum(mem_vec == 0)



      A <- igraph::modularity(network, mem_vec,
                              resolution=0,weights=igraph::E(network)$weight)
      Q <- igraph::modularity(network, mem_vec,
                              resolution=1,weights=igraph::E(network)$weight)
      # Note that the above E(network)$weight calls return NULL for unweighted graphs, which is then the proper, default input to the modularity function.
      mod_matrix[k-1, "base"] <- A
      mod_matrix[k-1, "decrement"] <- (A-Q)
    }
  ...
  # NEED ANOTHER SPLIT WORKFLOW FOR THE DIFFERENT INPUT OBJECTS
  ...
  if (partitions_type == "get_partitions") {
    ...
    modularity <- array(NA, dim = c(length(gammas), ncol(membership_df)-1))
    for (k in 1:(ncol(membership_df)-1)) {
      for (g in 1:length(gammas)) {
        modularity[g,k] <- a[[k]]-(gammas[g]*p[[k]])
      }
    }
  ...
  if (partitions_type == "get_partitions") {
    segments$partition_label <- segments$partitions
  } else {
    segments$partition_label <- stringr::str_replace_all(colnames(membership_df)[corresponding_partitions + 1], "_membership", "")
  }
  ...
  ggfig <- ggfig +
    ...
    ggplot2::geom_text(data = segments,
                       ...
                       color = as.numeric(as.factor(segments$partitions)),
                       vjust = -.5) +
  ...
  for (x in 1:nrow(partition_summary)) {
    ...
    if (partitions_type == "get_partitions") {
      ...
      mem_vec <- membership_df[, segments[x,"partitions"]+1]

      if (0 %in% mem_vec) {
        mem_vec <- mem_vec + 1
      }

      partition_summary$num_communities[x] <- max(mem_vec)
    }

  }

  # Let users know which method is best, if using `comm_detect` output
  if (partitions_type != "get_partitions") {
    partition_summary$method <- colnames(membership_df)[partition_summary$partition_num + 1]
    }

  #partition_summary <- partition_summary[order(-partition_summary$gamma_range),]

  if (partitions_type == "joint") {
    partitions <- list(cd_object = comm_detect_obj,
                       gp_object = get_part_obj,
                       CHAMPsummary = partition_summary,
                       CHAMPfigure = ggfig)
  } else {
