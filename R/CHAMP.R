...
#'
#' @returns \code{CHAMP} returns the input list of partitions with a \code{$CHAMPsummary} about which partitions are somewhere optimal (in the sense of modularity Q with a resolution parameter gamma) and their domains of optimality, along with the generated \code{$CHAMPfigure} plot of the upper envelope of Q(gamma).
#' 
#' @export
#' 
#' @examples
...
#PJM: 3.12.2025. Folding in Tom's suggested edits, as part of checking everything works with the new get_partitions that includes comm_detect results, and removed partitions that are only optimal for Q(gamma)<0
