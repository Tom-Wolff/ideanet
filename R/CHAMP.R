...
#' @param plots A logical value indicating whether output plots should be generated.
#'
...
#' @import igraphdata
#'
#' @author Peter J. Mucha (\email{peter.j.mucha@dartmouth.edu}), Sayan Bhattacharya, Alex Craig, Rachel Matthew, Sydney Rosenbaum, Ava Scharfstein and Joanna Shan
#'
#' @export
#'
#' @examples
...
#PJM: 12.5.2025. Changing calculation to faster version developed and tested by Joanna Shan and Sayan Bhattacharya.
...
CHAMP <- function( network,
  ...
  if (igraph::is_directed(network)) {
    stop("Input is directed. Only undirected networks are currently supported.")
  }
  
  # Force unit weights onto unweighted graph, for convenience
  if (!igraph::is_weighted(network)) {
    igraph::E(network)$weight <- 1
  }

  # "mod_matrix" is the intercept and slope of Q(gamma) line for each partition
  ...
  for (x in 1:length(partitions$partitions)) {
    # get memberships for this partition
    mem <- igraph::membership(partitions$partitions[[x]])
    
    # calculate sums of degrees (strengths actually) over each community
    k_c <- tapply(k, mem, sum)
    k2.c <- sum(k_c^2)
    
    # calculate sums of within-community edge weights
    internal_edge_indices <- mem[el[, 1]] == mem[el[, 2]]
    m.in <- sum(weights[internal_edge_indices])
    
    mod_matrix[x, "base"] <- m.in / m
    mod_matrix[x, "decrement"] <- k2.c / ((2 * m)^2)
  }
  ...
  # note the added 10^-8 boost is necessary to ensure we are PAST the end of the current mmp's range.
  ...
  fmax <- find_max_for_CHAMP(mod_matrix, 
                             c(partitions$gamma_min,partitions$gamma_max))

  # Now for plotting purposes we re-gather results at equispaced gamma values (there should be a nicer way to do this, but it's a very quick lookup and copy, so not a computational problem here to just do it this way)
  gammas <- seq(partitions$gamma_min,partitions$gamma_max,
                (partitions$gamma_max-partitions$gamma_min)/200)
  a <- list(); p <- list(); k <- 0
  for (partition in partitions$partitions) {
    k <- k+1
    a[k] <- mod_matrix$base[k]
    p[k] <- mod_matrix$decrement[k]
  }
  ...
  for (g in 1:length(best_gammas)) {
    ...
    best_modularities[g] <- a[[partition_index]] - 
      (best_gammas[g] * p[[partition_index]])
  }
    ...
    ggfig <- ggfig +
      ggplot2::geom_line(data = all,
                         mapping = ggplot2::aes(x = .data$x,
                                                y = .data$value,
                                                group = .data$partition_num),
                         show.legend = FALSE,
                         color = all$partition_num,  # still OK here
                         alpha = .3,
                         na.rm = TRUE) +
      ...
      ggplot2::geom_text(data = segments,
                         ggplot2::aes(x = (.data$x1 + .data$x2) / 2,
                                      y = (.data$y1 + .data$y2) / 2,
                                      label = .data$partitions),
                         color = segments$partitions,  # still OK here
                         vjust = -.5) +
      ggplot2::geom_segment(data = best,
                            mapping = ggplot2::aes(x = .data$best_gammas,
                                                   xend = .data$best_gammas,
                                                   y = .data$best_modularities,
                                                   yend = -Inf),
                            linetype = "dashed",
                            color = "black",
                            na.rm = TRUE) +
  ...
  colnames(partition_summary) <- c("starting_gamma", "ending_gamma",
                                   ...
                                   "next_gamma", "next_partition_num",
                                   "next_num_communities",
