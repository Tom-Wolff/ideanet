...
### Upload Node  and Edge Data ----
  ...
  node_data <- shiny::reactive({
    ...
    path_obj <- input$raw_nodes$datapath

    # If `raw_nodes` path is defined...
    if (is.character(path_obj)) {
        # Reading CSV
        if (input$select_file_type_edges == "csv") {
          # network edgelist
...
color_generator_edges <- shiny::reactive({
  ...
  if (input$palette_input == 'Uniform') {
    ...
    if (number_of_color_groups_edges() == 1) {
      grDevices::heat.colors(10)[5]
      palette <- grDevices::heat.colors(10)[5]
    } else {
...
### Visualize network ----
  ...
  output$measure_chooser <- shiny::renderUI({
    shiny::selectInput(inputId = "measure_chooser", label = "Choose Measures Output", choices = c("System", "Node"), selected = "System", multiple = FALSE)
  })
...
### Setup Analysis Tab ----
  output$analysis_chooser <- shiny::renderUI({
    shiny::selectInput(inputId = "analysis_chooser", label = "Choose Measures Output", choices = c("QAP", "Role Detection"), selected = "QAP", multiple = FALSE)
  })
...
#### Role Detection ----
  ...
  output$select_role_type <- shiny::renderUI({
    shiny::selectInput('select_role_type', label = "Choose Role Detection Type", choices = c('concor','cluster'))
  })
  ...
  output$select_role_viz <-
    shiny::renderUI({
      shiny::selectInput('select_role_viz', label = 'Choose Role Output Statistic', choices = role_detect_choices())
    })
